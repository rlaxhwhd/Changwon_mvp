...
| **DB 구조·데이터 소유권·이관 전제** | **`DB.md`** |
| 현행이 왜 그렇게 돼 있나 (근거) | `docs/_analysis/` · `docs/DB_CURRENT.html` |
| 아직 못 정한 것 | `DB.md` §9 **(미결 대장 — 여기 하나뿐)** |
| 구현 이력·설계 결정·버그 수정 | `Progress.md` |
...
| 단일소스 | 위치 | 담는 것 |
|---|---|---|
| **학생** | `src_v2/data/students/*.json` | 프로필·진단·IAP·로드맵·성장·포트폴리오·벌점·상담신청 |
...
| IAP 유형 확정 | 학생 IAP | `dc_iap_result` |
