...
| 단일소스 | 정본 | 위치 | 담는 것 |
|---|---|---|---|
| **학생** | ✅ DB | `dc.student` · `dc.student_list` 뷰 (시드 `backend/seeds/v2/students/*.json` · `studentsRoster.json`) | 프로필·진단·유형·로드맵·성장·포트폴리오·벌점·상담신청. **유형은 `student_type_event`, 이행률은 `roadmap_progress` 에서만 파생** — 시드의 더미 값은 읽지 않는다 |
...
| 심리검사 결과 작성 | ❌ localStorage | `dc_psych_tests` — **유일하게 남은 localStorage 업무 데이터**(`DB.md` §8-3 도메인 6) |
