...
- **진단 결과 연계 방식 확정(2026-09-14): 다른 개별 페이지에서 진단을 진행한 뒤, 외부에 저장된 진단 결과를 API로 가져와 우리 DB에도 저장하고 활용한다.** 외부 DB 직접 접속이나 화면에서만 일시 조회하는 방식으로 구현하지 않는다. 이번 지시는 메모이며 연계 구현 착수 지시가 아니다.
- API 주소·인증 방식·조회 시점, 학생 식별자 매핑, 결과표 형식, 결과 갱신·중복 처리 규격은 아직 정하지 않았다. 구체적인 계약이나 계산식을 임의로 만들지 않는다. 심리상담사의 수기 기록과 외부 CARE 7+ 진단 결과 연계를 혼동하지 않는다.
