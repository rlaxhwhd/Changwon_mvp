...
const programs = [
  { name: '취업특강 - IT PM 직무 이해', date: '2026-03-12', hours: 4, status: '수료' },
  { name: '캡스톤 디자인 중간 발표', date: '2026-04-02', hours: 8, status: '수료' },
  { name: '모의면접 프로그램 (A조)', date: '2026-04-09', hours: 2, status: '수료' },
  { name: 'SQLD 자격증 대비 스터디', date: '2026-02-15', hours: 20, status: '수료' },
  { name: '글로벌 어학 캠프 (TOEIC)', date: '2026-05-10', hours: 30, status: '진행중' },
];

export default function AiEvaluation({ onToast }: AiEvaluationProps) {
  ...
  const completed = programs.filter(p => p.status === '수료').length;

  return (
    <div>
      <div className="page-header">
        <h1>역량프로그램현황</h1>
        <p>참여한 역량개발 프로그램과 AI 종합평가 결과입니다</p>
      </div>

      {/* 상단 통계 카드 */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 20 }}>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 6 }}>총 참여 프로그램</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#6366F1' }}>{programs.length}건</div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 6 }}>수료 완료</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#22C55E' }}>{completed}건</div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 6 }}>총 이수 시간</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#F59E0B' }}>{totalHours}시간</div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-title">
          <i className="fa-solid fa-list-check" style={{ color: '#6366F1' }} /> 참여 프로그램 내역
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {programs.map((p, i) => (
            ...
            >
              <div>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#111827', marginBottom: 4 }}>{p.name}</div>
                <div style={{ fontSize: 12, color: '#6B7280' }}>{p.date} · {p.hours}시간</div>
              </div>
              <span className="badge" style={{
                background: p.status === '수료' ? '#F0FDF4' : '#EEF2FF',
                color: p.status === '수료' ? '#16A34A' : '#4F46E5',
                fontSize: 11,
              }}>{p.status}</span>
            </div>
          ))}
