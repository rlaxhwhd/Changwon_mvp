...
const NAV: NavItem[] = [
  { label: 'AI 커리어 라운지', path: '/lounge' },
  {
    ...
    children: [
      ...
      { label: '비교과 프로그램 신청', path: '/growth/program' },
      { label: '퀘스트보드', path: '/growth/quest' },
      { label: '성장 경험일지', path: '/growth/journal' },
  ...
  },
  {
    label: '진단센터',
    children: [
      { label: '취업지원 역량진단', path: '/diagnosis/employment' },
  ...
  { label: '취업지원', path: '/jobs' },
  {
    ...
    children: [
      ...
      { label: '비교과 프로그램 현황', path: '/mypage/programs' },
      { label: '상담 현황', path: '/mypage/counsel' },
...
export default function GNB() {
  ...
  return (
    <header className="gnb">
      <Link to="/lounge" className="gnb-logo" aria-label="CWNU 홈">
      ...
      <nav className="gnb-nav" aria-label="주요 메뉴">
        {NAV.map(item => (
          <div className="gnb-nav-item" key={item.label}>
            {item.path ? (
              <Link to={item.path} className={`gnb-nav-link ${isActive(item) ? 'active' : ''}`}>
                {item.label}
              </Link>
            ) : (
              <>
