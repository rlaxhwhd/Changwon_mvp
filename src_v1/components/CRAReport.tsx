...
const sectionTitle: React.CSSProperties = { fontSize: 18, fontWeight: 700, marginBottom: 16, paddingBottom: 8, borderBottom: '3px solid #F59E0B' };
const subTitle: React.CSSProperties = { fontSize: 14, fontWeight: 600, color: '#6B7280', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 };
const tbl: React.CSSProperties = { width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 16 };
const th: React.CSSProperties = { background: '#F3F4F6', padding: '8px 12px', border: '1px solid #E5E7EB', fontWeight: 600, textAlign: 'center' };
const thPurple: React.CSSProperties = { ...th, background: '#EDE9FE', color: '#4F46E5' };
const td: React.CSSProperties = { padding: '8px 12px', border: '1px solid #E5E7EB', textAlign: 'center' };
...
function ScoreBadge({ level }: { level: string }) {
  ...
  return <span style={{ background: bg, color, padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600 }}>{level}</span>;
}
  ...
  return (
    <Modal open={open} onClose={onClose} title="CRA 진로준비도 진단검사 결과표" size="lg">
      <div style={{ maxHeight: '70vh', overflowY: 'auto', padding: '0 4px' }}>
        {/* 상단 정보 */}
        <div style={{ background: '#F9FAFB', borderRadius: 12, padding: 20, marginBottom: 24, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 13 }}>
          <div><span style={{ color: '#6B7280' }}>검사일시</span> <strong>{examDate}</strong></div>
          <div><span style={{ color: '#6B7280' }}>소속대학</span> <strong>국립창원대학교</strong></div>
          <div><span style={{ color: '#6B7280' }}>학과</span> <strong>컴퓨터공학과</strong></div>
          <div><span style={{ color: '#6B7280' }}>학번</span> <strong>20250001</strong></div>
          <div><span style={{ color: '#6B7280' }}>학년</span> <strong>2학년</strong></div>
          <div><span style={{ color: '#6B7280' }}>성명</span> <strong>김민준</strong></div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={{ textAlign: 'center', padding: 16, background: '#F9FAFB', borderRadius: 8 }}>
            <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 8 }}>자신의 유형</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#4F46E5' }}>역량성장형</div>
          </div>
          <table style={tbl}>
            <thead><tr><th style={thPurple}>유형</th><th style={thPurple}>수준</th><th style={thPurple}>T점수</th></tr></thead>
            <tbody>
        ...
        <table style={tbl}>
          ...
          <tbody>
            ...
            <tr style={{ background: '#EDE9FE' }}><td style={{ ...td, fontWeight: 700 }}>역량성장형</td><td style={td}>Action 집중형</td><td style={tdLeft}>진로목표도 어느 정도 있고 역량도 개발하고 있으나 취업준비가 부족함</td><td style={tdLeft}>
        ...
        <table style={tbl}>
          ...
          <tbody>
            ...
            <tr style={{ background: '#FEF3C7' }}><td style={{ ...td, fontWeight: 700 }}>미래 유보형</td><td style={td}><i className="fa-solid fa-arrow-down" style={{ color: '#EF4444' }} /></td><td style={td}><i className="fa-solid fa-arrow-down" style={{ color: '#EF4444' }} /></td><td style={tdLeft}>진로 목표도 불분명하고, 열정이나 의지도 낮은 상태입니다.</td></tr>
          </tbody>
        </table>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={{ background: '#F9FAFB', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 8 }}>유형 판정</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#F59E0B', marginBottom: 12 }}>미래 유보형</div>
          ...
          <div style={{ background: '#F9FAFB', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>미래 유보형 강화 방안</div>
        ...
        <table style={tbl}>
          ...
          <tbody>
            ...
            <tr style={{ background: '#EDE9FE' }}><td style={{ ...td, fontWeight: 700 }}>직무열정 유형</td><td style={td}><i className="fa-solid fa-arrow-up" style={{ color: '#22C55E' }} /></td><td style={td}><i className="fa-solid fa-arrow-down" style={{ color: '#EF4444' }} /></td><td style={tdLeft}>직무 수행에 필요한 전문지식과 기술이 뛰어나나, 팀 내 협업이나 갈등 해결에 어려움이 있을 수 있습니다.</td></tr>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={{ background: '#F9FAFB', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 8 }}>유형 판정</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: '#4F46E5', marginBottom: 12 }}>직무열정 유형</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 13 }}>
          ...
          <div style={{ background: '#F9FAFB', borderRadius: 8, padding: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>직무열정 유형 보완 방안</div>
            <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.7 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <div style={{ textAlign: 'center', padding: 16, background: '#F9FAFB', borderRadius: 8 }}>
            <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 8 }}>강점 요인</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: '#22C55E' }}>면접역량</div>
          </div>
          <table style={tbl}>
            <thead><tr><th style={thPurple}>유형</th><th style={thPurple}>수준</th><th style={thPurple}>T점수</th></tr></thead>
            <tbody>
