...
> | 질문 | 절 |
> |---|---|
> | 어떤 화면·기능이 필요한가, 지금 상태는 | **§3** ← 현행 사이트맵 847화면 전수 대조 |
...
>
> **이 문서에 없는 것** — DB 구조·이관 전제·현행 원인 분석은 **`DB.md`**, 구현 이력·설계 결정·버그 수정은 **`Progress.md`**.
>
...
현행 화면 목록의 출처는 `docs/_analysis/01_sitemap.md`(매핑 1,600 / 화면 847) 전수 대조다.
...
> **구현 이력·설계 결정·버그 수정**은 `Progress.md`에 있다. 이 문서는 *무엇이 필요한가*와 *지금 어디까지 됐나*만 담는다.
...
| 표시 | 검사별 응시율 카드 4장 + 학생×검사 표(학번·이름·학과·학년·검사·상태·응시일·결과 요약·재검사) |
...
| ★ | **'미응시'는 저장하지 않는다.** (담당 학생 × 학년별 대상검사) 곱집합에서 응시 레코드를 뺀 나머지를 로더가 파생한다. 학년별 대상은 `careerProcess.DIAGNOSIS_BY_GRADE`(§7-10) |
| 참고 | **심리검사(§⑩)와 다른 도메인이다.** 진단 4종은 학생 단독 응시, 심리검사는 상담 건에 종속 |
...
| # | 기능 | 현행 | 상태 |
|---|---|---|---|
| S1 | 상담 신청 · 내역 | `CoVi010M`·`CoMs010L` | ✅ `/counsel/career`·`/psych`·`/record` |
...
| # | 기능 | 현행 | 상태 |
|---|---|---|---|
| S10 | 신청 · 내역 · 찜 | `EpMs010L`·`020L` | ✅ `/growth/program`·`/mypage/programs` |
...
**마일리지를 제외한 사실상 전 도메인이 현행 운영 데이터를 이관받는다.** 이것이 설계 자유도를 실질적으로 제한한다.

> **→ 전체 내용(도메인별 이관 O/X 표 · 강제사항 5가지)은 `DB.md` §8-0이 단일 소스다.**

화면·스키마를 만들 때 매번 걸리는 것만 여기 적는다.

1. **모든 코드에 `legacy` 값을 채운다** (§7-0 규칙 2). 비면 이관 스크립트를 쓸 수 없다.
...
**C-1은 존재하지 않는다** (번호는 C-2부터).
⚠️ **옛 명칭은 전부 폐기한다** — 유형분류 · 자기이해 · CARES · KVCT · E-DISC · SPRINT Ⅰ/Ⅱ · NEO. 화면·데이터 어디에도 쓰지 않는다.
...
**단일소스** — `src_v2/data/careerProcess.ts`
· 검사 목록 `DIAGNOSIS_MODULES` · **학년별 대상 `DIAGNOSIS_BY_GRADE` / `getGradeTests()`**
라운지 결과요약은 `AiLounge.tsx`, 상세 이름맵은 `DiagnosisResultDetail.tsx`.

**상태 규칙** — `CCORE`만 결과보기(`done`), 나머지는 검사시작(`available`).
학년별 선택/공통은 **정책**이며 현재 진단센터 UI는 4종을 모두 노출한다(학년 필터링 미구현 — §3-6 S7).
...
| 관문 | 증상 | 우리 대응 |
|---|---|---|
| **① 신분코드** | 대학원생이 **학생 목록·검색·배정 대상에서 사라진다** (`'1101'` 리터럴 229곳) | §7-2 `STUDENT_ENROLLED` 집합 상수로만 판정. **리터럴 금지** |
| **② 학과 트리** | 대학원 **학과 자체가 목록에 안 뜬다** | `V_DEP_INF_ALL` 사용 + `USE_YN`을 `ON`절로 + `ORG`(기관) 상위 허용 |

> **→ 원인 분석(막히는 화면 5종 · SQL 근거 · 파일별 `1101` 분포)은 `DB.md` §7이 단일 소스다.**
...
| 항목 | 결정 |
|---|---|
| 상담 가능시간 방식 | **제한(불가) 등록.** 기본값 전 시간대 가능 — 상담사·교수 공통(§3-1-⑦ · §3-5) |
...
### 10-2. AI 로드맵 3종 <span>★ 서로 다른 물건이다</span>

| | 화면 | 성격 | 사람이 손대나 | 저장 위치 |
|---|---|---|---|---|
| **①** | **AI 진로로드맵** `/roadmap/ai` | **실질 로드맵.** 단·중·장기를 AI와 상담사가 함께 설계 | ✅ 학생 변경요청 · 상담사 수정 | AI 초안 = `ai_roadmap(kind=진로)`<br>확정본 = **`dc_roadmap`**(서비스DB) |
| **②** | **AI 직무로드맵** `/roadmap/skill-tree` | **참고용 정보확인.** 직무 추천 + 적합도 | ❌ 읽기 전용 | `ai_job_fit` (AI DB만) |
| **③** | **종합로드맵** | ①의 확정본 + ②의 적합도를 합친 산출물 | ❌ 읽기 전용 | `ai_roadmap(kind=종합)` |

```
   ①  AI 초안 ──> 상담사 조정 ──> 확정      dc_roadmap (status: 초안/검토중/확정)
                       │                       변경은 dc_roadmap_history 에 append
                       ├───────────┐
   ②  수강·자격·진단 ──> 적합도 ──────┤        ai_job_fit  (참고용, 서비스DB 흔적 없음)
                                   ▼
   ③                          종합로드맵      ai_roadmap(kind=종합)
                                              base_roadmap_id · base_fit_run_id
```

> **①만 서비스DB에 실체를 갖는다.** 상담사가 고치고 학생이 변경을 요청하므로 조회·권한·이력이
> 필요하다. ②③은 읽기 전용 산출물이라 AI DB에만 있으면 된다.

#### ② `/roadmap/skill-tree` 의 정의

> **"학생의 기술역량 트리"가 아니다.** 라우팅에 `skill` 이 들어갔다고 기술스택 화면으로 읽으면
> 안 된다. **직무 추천 + 적합도를 보여주는 참고용 페이지**다.
...
| 4 | **보유 자격증** | `dc_student_cert` · `dc_lang_score` |

```
  네트워크 · 백엔드 · 자료구조 · 경영학 을 이수한 학생
        ↓ AI 분석
  IT PM        적합도 80%
  백엔드 개발자  적합도 70%
```

> ⚠️ **과목↔스킬 수기 매핑 테이블을 만들지 않는다.** 초기 설계에 `dc_course_skill`(수천 과목을
> 사람이 스킬에 매핑)을 뒀다가 폐기했다 — **AI가 과목 리스트를 직접 읽고 판단**하므로 중간층이
> 불필요하고, 매핑 자체가 운영 병목이 된다. 스킬 어휘(`dc_skill`·`dc_job_skill`)와
> 학생 보유스킬(`dc_student_skill`)도 같은 이유로 제거했다.
>
> ⚠️ **②는 읽기 전용이므로 "직무 담기" 기능이 없다.** 학생의 목표 직무는 `dc_career_goal.target_job`
> 하나가 정식 소스다. (`dc_student_job_interest` 를 뒀다가 중복이라 삭제)

#### ① 의 수정 이력

> ⚠️ **`dc_roadmap_override`(로드맵 전체를 payload JSON 한 덩어리로 덮어쓰기)를 폐기했다.**
> 프로토타입의 localStorage 오버레이 패턴을 DB 설계로 그대로 옮긴 오류였다. 그러면
> "무엇이 왜 바뀌었나"를 질의할 수 없고, 상담사가 둘이면 누구 payload 가 최신인지도 모호하다.
> **항목(`dc_roadmap_item`)을 직접 고치고 변경을 `dc_roadmap_history` 에 append 한다.**
...
| 화면 | 용도 | 상태 |
|---|---|---|
| `/roadmap/ai` | **① AI 진로로드맵** (§10-2) | 화면 ✅ / AI ❌ |
| `/roadmap/skill-tree` | **② AI 직무로드맵** (§10-2) | 화면 ✅ / AI ❌ |
| (미정) | **③ 종합로드맵** (§10-2) | 화면 ❌ / AI ❌ |
| 취업예측분석 · AI맞춤채용 | 합격 예측 · 기업 추천 | ❌ |
| AI 자소서 · 면접 | 작성 · 첨삭 | ❌ |
| 상담 기록 분석 | 상담기록지 → 요약·IAP 제안 | ❌ |
...
| 문서 | 용도 |
|---|---|
| `CLAUDE.md` | 작업 규칙 · 문서 라우팅 (**사실은 담지 않는다**) |
| **`DB.md`** | **이 문서의 근거** — 현행 DB 실측 · 소유권 경계 · 이관 전제 · 미결 대장 |
| `Progress.md` | 구현 이력 · 설계 결정 · 버그 수정 로그 |
