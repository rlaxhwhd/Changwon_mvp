...
| # | 기능 | 현행 | 상태 |
|---|---|---|---|
| S1 | 상담 신청 · 내역 | `CoVi010M`·`CoMs010L` | ✅ `/counsel/career`·`/psych`·`/record` — **C-CORE 미완료 시 잠금**: 비활성 상태와 “먼저 C-CORE 진단을 완료하세요” 안내·진단센터 링크를 표시 |
...
프로토타입은 JSON mock이지만 **실서비스 전환 대상은 Oracle이다.** JSON 단계에서 원칙을 지켜야 DB 전환이 로더 교체만으로 끝난다.
...
| `careerTopicCodes` | — | 진로취업 상담에서 선택한 해당 유형의 4개 주제 코드 (`A01`~`A24`, §7-10). 심리·교수상담에는 적용하지 않음 |
| `finalStudentType` | — | C-CORE 결과와 후속진단을 바탕으로 상담 중 최종 확정한 6유형 |
| `roadmapId` | — | 같은 오프라인 상담에서 생성한 확정 로드맵. 진로취업 상담 완료 시 필수 |
