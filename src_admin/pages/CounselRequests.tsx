...
function ScheduleAction({ request, onClose }: { request: CounselRequest; onClose: () => void }) {
  ...
  const save = () => { if (!valid) return; const slot: CounselSlot = { date, start, end, place: place.trim() || undefined }; request.status === '확정' ? rescheduleRequest(request.id, slot) : confirmRequest(request.id, slot); window.location.reload() }
  const cancel = () => { if (!cancelReason.trim()) return; rejectRequest(request.id, cancelReason.trim()); window.location.reload() }
  return <div className="counsel-modal-action"><div className="counsel-request-detail-grid"><label><span>날짜</span><input type="date" value={date} onChange={e => setDate(e.target.value)} /></label><label><span>시작</span><input type="time" value={start} onChange={e => setStart(e.target.value)} /></label><label><span>종료</span><input type="time" value={end} onChange={e => setEnd(e.target.value)} /></label><label className="is-wide"><span>장소 / 링크</span><input value={place} onChange={e => setPlace(e.target.value)} /></label>{cancelling && <label className="is-wide"><span>취소 사유 <em className="counsel-required">필수</em></span><textarea rows={2} value={cancelReason} onChange={e => setCancelReason(e.target.value)} placeholder="취소 사유를 입력하세요. 처리 이력에 기록됩니다." /></label>}</div><AdminFormActions>{request.status === '대기' && (cancelling ? <><button type="button" className="admin-btn admin-btn-ghost" onClick={() => { setCancelling(false); setCancelReason('') }}>취소 중단</button><button type="button" className="admin-btn admin-btn-danger-ghost" disabled={!cancelReason.trim()} onClick={cancel}>취소 확정</button></> : <button type="button" className="admin-btn admin-btn-danger-ghost" onClick={() => setCancelling(true)}>신청 취소</button>)}<button type="button" className="admin-btn admin-btn-ghost" onClick={onClose}>닫기</button><button type="button" className="admin-btn admin-btn-primary" disabled={!valid} onClick={save}>{request.status === '확정' ? '일정 변경' : '상담 확정'}</button></AdminFormActions></div>
}
...
function ReassignAction({ request, onClose }: { request: CounselRequest; onClose: () => void }) {
  ...
  return <div className="counsel-modal-action"><div className="counsel-request-detail-grid"><label className="is-wide"><span>재배정할 상담사</span><select value={selectedId} onChange={e => setSelectedId(e.target.value)}>{candidates.map(c => <option key={c.id} value={c.id}>{c.name} · {c.roleLabel}</option>)}</select></label><label className="is-wide"><span>재배정 사유 <em className="counsel-optional">선택</em></span><textarea rows={2} value={reason} onChange={e => setReason(e.target.value)} placeholder="이관 사유를 남기면 처리 이력에 함께 기록됩니다." /></label></div><AdminFormActions><button type="button" className="admin-btn admin-btn-ghost" onClick={onClose}>닫기</button><button type="button" className="admin-btn admin-btn-primary" disabled={!selectedId} onClick={() => { reassignRequest(request.id, selectedId, reason); window.location.reload() }}>재배정</button></AdminFormActions></div>
}
...
const initialKey = useMemo(() => pickReferenceDate(all.map(requestDate), dateKey(new Date())), [all])
  ...
  return <div className="admin-page counsel-requests-page"><header className="admin-page-head"><div><h1 className="admin-page-title">신청 접수함</h1><p className="admin-page-desc">학생들이 신청한 상담 요청을 확인하고 관리할 수 있습니다.</p></div></header><form className="counsel-filter-bar" onSubmit={event => event.preventDefault()}><label className="counsel-filter-search"><LuSearch aria-hidden="true" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="이름, 학번 검색" aria-label="이름 또는 학번 검색" /></label><select className="counsel-filter-select" value={typeFilter} onChange={event => setTypeFilter(event.target.value as '전체' | CounselRequestType)} aria-label="상담 유형"><option value="전체">전체 유형</option><option value="진로취업">진로취업 상담</option><option value="심리">심리상담</option></select><input type="date" className="counsel-filter-date" value={selectedDate} onChange={event => updateSelectedDate(event.target.value)} aria-label="상담 신청 날짜" /><button type="submit" className="admin-btn admin-btn-primary"><LuSearch /> 검색</button><button type="button" className="admin-btn admin-btn-ghost" onClick={() => { setQuery(''); setTypeFilter('전체'); setTab('전체'); updateSelectedDate(initialKey) }}><LuRotateCcw /> 초기화</button></form><div className="admin-tabs" role="tablist" aria-label="상담 신청 상태">{TABS.map(item => <button key={item} type="button" role="tab" aria-selected={tab === item} className={`admin-tab${tab === item ? ' active' : ''}`} onClick={() => setTab(item)}><span>{item}</span><span className="admin-tab-count">{counts[item]}</span></button>)}</div><div className="counsel-requests-layout"><section className="counsel-calendar-card" aria-label="상담 신청 달력"><h2>상담 신청 캘린더</h2><div className="counsel-calendar-toolbar"><div className="counsel-calendar-nav"><button type="button" aria-label="이전 달" onClick={() => setMonth(current => new Date(current.getFullYear(), current.getMonth() - 1, 1))}><LuChevronLeft /></button><button type="button" aria-label="다음 달" onClick={() => setMonth(current => new Date(current.getFullYear(), current.getMonth() + 1, 1))}><LuChevronRight /></button></div><strong>{month.getFullYear()}년 {month.getMonth() + 1}월</strong><button type="button" className="counsel-today-btn" onClick={() => { const today = new Date(); setMonth(new Date(today.getFullYear(), today.getMonth(), 1)); setSelectedDate(dateKey(today)) }}>오늘</button></div><div className="counsel-calendar-weekdays">{WEEKDAYS.map(day => <span key={day}>{day}</span>)}</div><div className="counsel-calendar-grid">{days.map(day => { const count = dateCounts.get(day.key) ?? 0; return <button key={day.key} type="button" className={`${day.inMonth ? '' : 'is-muted'}${day.key === selectedDate ? ' is-selected' : ''}`} onClick={() => updateSelectedDate(day.key)}><span>{day.day}</span>{count > 0 && <em>{count}</em>}</button> })}</div><p className="counsel-calendar-legend"><span /> 해당 날짜의 상담 신청 건수</p></section><section className="counsel-request-table-card"><header className="counsel-request-table-head"><h2>{formatSelectedDate(selectedDate)} <span>{view === 'schedule' ? '상담 일정' : '상담 신청 목록'}</span> <em>{view === 'schedule' ? dayCount : list.length}건</em></h2><div><button type="button" className={`admin-btn ${view === 'schedule' ? 'admin-btn-primary' : 'admin-btn-ghost'}`} aria-pressed={view === 'schedule'} onClick={() => setView(current => (current === 'schedule' ? 'list' : 'schedule'))}>{view === 'schedule' ? <><LuListChecks /> 신청 목록</> : <><LuCalendarDays /> 상담일정</>}</button><button type="button" className="admin-btn admin-btn-ghost"><LuFilter /> 필터</button><button type="button" className="admin-btn admin-btn-ghost" onClick={downloadCsv}><LuDownload /> 엑셀 다운로드</button></div></header>{view === 'schedule' ? <div className="counsel-day-schedule">{daySchedule.length === 0 ? <div className="counsel-request-empty"><LuCalendarX /><strong>이 날짜에 잡힌 상담 일정이 없습니다.</strong><span>예약이 잡히거나 가능 시간대를 설정하면 이곳에 시간순으로 나타납니다.</span></div> : daySchedule.map(row => (
                    ...
                    {row.items.map(req => (
                      <article key={req.id} className="counsel-day-item">
                        ...
                        <div className="counsel-day-side">
                          ...
                          <button type="button" className="admin-btn admin-btn-ghost sm" onClick={() => setInfoId(req.studentId)}>상세 보기</button>
                          {/* 학생이 낸 신청서를 상담 전에 읽는다 — 학생이 본 서식 그대로(공용 모달). */}
                          <button type="button" className="admin-btn admin-btn-ghost sm" onClick={() => setIntakeReq(req)}>문진표 확인</button>
                          {/* 대기는 확정부터 — 목록의 「상담 처리」와 같은 모달을 연다(처리 경로를 둘로 만들지 않는다). */}
                          {req.status === '대기' && <button type="button" className="admin-btn admin-btn-ghost sm" onClick={() => openModal(req)}>확정하기</button>}
                          {req.status === '확정' && <Link to={`/counsel/session/${req.studentId}`} className="admin-btn admin-btn-ghost sm">상담 진행</Link>}
                        </div>
                      </article>
                    ))}
                  </div>
                ...
                <div className="counsel-request-row-wrap" key={req.id}>
                  <article className="counsel-request-row">
                    ...
                    <div className="counsel-request-cell counsel-request-manage">
                      <button type="button" className="admin-btn admin-btn-ghost sm" onClick={() => setInfoId(req.studentId)}>상세 보기</button>
                      {canAct && <button type="button" className="admin-btn admin-btn-ghost sm" onClick={() => openModal(req)}>상담 처리</button>}
                    </div>
                  </article>
