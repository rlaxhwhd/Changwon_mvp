...
> 프론트엔드 전용 · 백엔드 없음 · 모든 데이터는 가능하면 json으로 동적이게 mock · **모바일 반응형**.
>
...
| 뷰포트 | **모바일 반응형** <span>(2026-09-08 — 데스크톱 전용에서 전환)</span><br>셸 브레이크포인트는 `index.css` 가 갖는다 — 1240 `body min-width` 해제 · 900 GNB 햄버거.<br>규약은 `CLAUDE.md` §구현 규약이 단일 소스다 |
| 언어 | 한국어 UI |
